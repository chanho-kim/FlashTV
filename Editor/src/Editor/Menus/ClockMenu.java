...
public class ClockMenu extends JFrame implements ActionListener, WindowFocusListener{
	...
	public void getComponent(Clock clock){
		this.clock = clock;
		label.setText(clock.getText());	
		
		bgColor.setSelectedIndex(Menu.determineBgColorIndex2(clock.getBackground()));	
		txtFont.setSelectedIndex(Menu.determineFtIndex(clock.getFont()));
		txtColor.setSelectedIndex(Menu.determineFgColorIndex(clock.getForeground()));
		txtSize.setSelectedItem(clock.getFont().getSize());
		boldOption.setSelected(Menu.determineBold(clock.getFont()));
		italic.setSelected(Menu.determineItalic(clock.getFont()));
	}
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource().equals(confirm)){
			confirmed  = true;
			clock.setText(label.getText());
			Workboard.wrapText(clock);
			setBgColor();
			setTextColor();
			setFont();
	...
	private void setBgColor(){
		clock.setBackground(Menu.determineColor(this.bgColor.getSelectedIndex()));
	}
	

	private void setTextColor(){
		clock.setForeground(Menu.determineColor(this.txtColor.getSelectedIndex()));
	}
	
	private void setFont(){
		Object a = txtSize.getSelectedItem();
		if(a instanceof Integer){
			clock.setFont(Menu.determineFont((int)a,this.txtFont.getSelectedIndex(), this.boldOption.isSelected(),this.italic.isSelected()));
		}	
		else{
			try{
				int b = Integer.parseInt((String)a);
				clock.setFont(Menu.determineFont(b,this.txtFont.getSelectedIndex(), this.boldOption.isSelected(),this.italic.isSelected()));
			}catch(Exception e){
